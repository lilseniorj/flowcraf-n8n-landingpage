...
export const WORKFLOWS = [
  ...
  // ──────────────────────────────────────────────────
  {
    ...
    canvas: [
      ...
      { id: "n7", x: 1000, y: 80,  type: "app",     label: "Gmail Info",     icon: "📧", desc: { es: "Envía email automático con información del producto al lead que pidió info.", en: "Sends automatic product information email to the lead who requested info." } },
      { id: "n8", x: 1000, y: 160, type: "ai",      label: "Sentimiento",    icon: "🧠", desc: { es: "LLM Chain con GPT-4o-mini que analiza el sentimiento: Positive, Negative o Aggressive.", en: "LLM Chain with GPT-4o-mini that analyzes sentiment: Positive, Negative, or Aggressive." } },
      { id: "n9", x: 1200, y: 80,  type: "if",      label: "Ruta Sentim.",   icon: "🔀", desc: { es: "Switch que clasifica al lead de soporte por tono: positivo envía respuesta IA, negativo y agresivo generan alertas especiales.", en: "Switch classifying support lead by tone: positive sends AI response, negative and aggressive generate special alerts." } },
    ],
    edges: [
  ...
  // ──────────────────────────────────────────────────
  {
    ...
    canvas: [
      ...
      { id: "n4", x: 400,  y: 80,  type: "app",     label: "Notion (Legal)",   icon: "📓", desc: { es: "Crea página en base de datos Notion 'Legal Request' con estado 'en tramite', fecha y mensaje.", en: "Creates page in Notion 'Legal Request' database with status 'en tramite', date and message." } },
      { id: "n5", x: 400,  y: 160, type: "app",     label: "Gmail (Facilities)",icon: "📧",desc: { es: "Envía email al responsable de Facilities con los detalles de la solicitud.", en: "Sends email to the Facilities manager with request details." } },
      { id: "n6", x: 400,  y: 240, type: "app",     label: "Sheets (Otro)",    icon: "📊", desc: { es: "Registra la petición en Google Sheets como fallback para categorías no reconocidas.", en: "Registers the request in Google Sheets as fallback for unrecognized categories." } },
  ...
  // ──────────────────────────────────────────────────
  {
    ...
    connections: 4,
    canvas: [
      { id: "n1", x: 0,   y: 64, type: "trigger", label: "MCP Trigger",   icon: "🔌", desc: { es: "MCP Server Trigger en el path '/herramientas' que expone las herramientas a cualquier agente compatible con MCP.", en: "MCP Server Trigger at path '/herramientas' that exposes tools to any MCP-compatible agent." } },
      { id: "n2", x: 200, y: 0,  type: "app",     label: "Fecha/Hora",   icon: "🕐", desc: { es: "DateTimeTool que devuelve la fecha y hora actuales al agente que lo invoca.", en: "DateTimeTool that returns the current date and time to the invoking agent." } },
      { id: "n3", x: 200, y: 80, type: "app",     label: "Crear Evento", icon: "📅", desc: { es: "Google Calendar Tool que agenda eventos con título descriptivo, asistentes y enlace Google Meet automático.", en: "Google Calendar Tool that schedules events with descriptive title, attendees and automatic Google Meet link." } },
      { id: "n4", x: 200, y: 160,type: "http",    label: "SerpAPI",      icon: "🔎", desc: { es: "HTTP Request Tool que consulta SerpAPI para búsquedas web en tiempo real desde el agente IA.", en: "HTTP Request Tool that queries SerpAPI for real-time web searches from the AI agent." } },
      { id: "n5", x: 200, y: 240,type: "app",     label: "Enviar Email", icon: "📧", desc: { es: "Gmail Tool que permite al agente enviar emails con destinatario, asunto y cuerpo generados por IA.", en: "Gmail Tool that allows the agent to send emails with AI-generated recipient, subject and body." } },
    ],
  ...
  // ──────────────────────────────────────────────────
  {
    ...
    connections: 4,
    canvas: [
      { id: "n1", x: 0,   y: 64, type: "trigger", label: "Chat Trigger",  icon: "💬", desc: { es: "Chat Trigger de n8n que inicia la sesión conversacional con el agente.", en: "n8n Chat Trigger that starts the conversational session with the agent." } },
      { id: "n2", x: 200, y: 64, type: "ai",      label: "AI Agent",      icon: "🤖", desc: { es: "Agente principal con GPT-4o-mini que coordina el uso de memoria y herramientas para responder.", en: "Main agent with GPT-4o-mini that coordinates the use of memory and tools to respond." } },
      { id: "n3", x: 400, y: 0,  type: "ai",      label: "GPT-4o-mini",   icon: "🧠", desc: { es: "LLM OpenAI GPT-4o-mini conectado como modelo de lenguaje del agente.", en: "OpenAI GPT-4o-mini LLM connected as the agent's language model." } },
      { id: "n4", x: 400, y: 80, type: "ai",      label: "Memoria",       icon: "🧩", desc: { es: "Buffer Window Memory que mantiene el historial de la conversación en una ventana deslizante.", en: "Buffer Window Memory that maintains conversation history in a sliding window." } },
      { id: "n5", x: 400, y: 160,type: "http",    label: "SerpAPI",       icon: "🔎", desc: { es: "SerpAPI Tool que permite al agente buscar en Google en tiempo real para respuestas actualizadas.", en: "SerpAPI Tool that allows the agent to search Google in real-time for up-to-date answers." } },
    ],
